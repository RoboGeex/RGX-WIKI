...
export default function TrackManager({ audience }: { audience: 'teacher' | 'admin' }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {editorOpen && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/55 p-4 backdrop-blur-sm" role="dialog" aria-modal="true"><form onSubmit={save} className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-2xl bg-white shadow-2xl"><div className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-200 bg-white px-6 py-4"><div><h2 className="text-xl font-extrabold text-slate-950">{editingId ? 'Edit track' : 'Create a track'}</h2><p className="text-sm text-slate-500">Choose wikis, set their order, and assign students.</p></div><button type="button" onClick={() => setEditorOpen(false)} className="rounded-lg p-2 text-slate-400 hover:bg-slate-100"><X size={20} /></button></div><div className="space-y-7 p-6">
        {error && <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-700">{error}</div>}
        <div className="grid gap-4 sm:grid-cols-2"><label className="sm:col-span-1"><span className="mb-1.5 block text-sm font-bold text-slate-700">Track name</span><input required value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} className="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm outline-none focus:border-[#E94335]" placeholder="Robotics foundations" /></label><label className="sm:col-span-1"><span className="mb-1.5 block text-sm font-bold text-slate-700">Description <span className="font-normal text-slate-400">(optional)</span></span><input value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} className="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm outline-none focus:border-[#E94335]" placeholder="A guided learning path" /></label></div>
        <section><div className="mb-3 flex items-center justify-between"><div><h3 className="font-extrabold text-slate-900">Wikis</h3><p className="text-xs text-slate-500">Select at least one. Use arrows to set the track order.</p></div><span className="text-xs font-bold text-[#E94335]">{form.wikiSlugs.length} selected</span></div><div className="grid gap-2 sm:grid-cols-2">{wikis.map((wiki) => { const selected = form.wikiSlugs.includes(wiki.slug); const index = form.wikiSlugs.indexOf(wiki.slug); return <div key={wiki.slug} className={`flex items-center gap-2 rounded-lg border p-2.5 ${selected ? 'border-[#E94335] bg-red-50/50' : 'border-slate-200'}`}><button type="button" onClick={() => toggle('wikiSlugs', wiki.slug)} className="flex min-w-0 flex-1 items-center gap-2 text-left"><span className={`flex h-5 w-5 shrink-0 items-center justify-center rounded border ${selected ? 'border-[#E94335] bg-[#E94335] text-white' : 'border-slate-300'}`}>{selected && <Check size={13} />}</span><span className="truncate text-sm font-bold text-slate-700">{wiki.displayName}</span></button>{selected && <><span className="text-xs font-extrabold text-slate-400">#{index + 1}</span><button type="button" onClick={() => moveWiki(index, -1)} disabled={index === 0} className="p-1 text-slate-500 disabled:opacity-20"><ChevronUp size={15} /></button><button type="button" onClick={() => moveWiki(index, 1)} disabled={index === form.wikiSlugs.length - 1} className="p-1 text-slate-500 disabled:opacity-20"><ChevronDown size={15} /></button></>}</div> })}</div></section>
      ...
      </div><div className="sticky bottom-0 flex items-center justify-end gap-2 border-t border-slate-200 bg-white px-6 py-4"><button type="button" onClick={() => setEditorOpen(false)} className="rounded-lg border border-slate-300 px-4 py-2.5 text-sm font-bold text-slate-600">Cancel</button><button disabled={saving} className="inline-flex items-center gap-2 rounded-lg bg-[#E94335] px-5 py-2.5 text-sm font-bold text-white disabled:opacity-60">{saving && <Loader2 size={15} className="animate-spin" />}{editingId ? 'Save changes' : 'Create track'}</button></div></form></div>}
    </div>
  )
}
