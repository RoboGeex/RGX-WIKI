...
/**
 * The public wiki catalog, rendered at BOTH the hub root (/) and /wikis.
 ...
 * redirect to that wiki, not show the catalog).
 */
...
type CatalogPageProps = {
  locale?: 'en' | 'ar'
  initialQuery?: string
  /** Where the language toggle should point (differs between / and /wikis). */
  basePath?: string
}
      ...
      <header className="sticky top-0 z-50 border-b border-ink/10 bg-white/95 backdrop-blur">
        <div className="mx-auto flex w-full max-w-7xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
          ...
          <nav className="flex items-center gap-2 sm:gap-4">
            ...
            <Link
              href={`${basePath}${basePath.includes('?') ? '&' : '?'}lang=${locale === 'ar' ? 'en' : 'ar'}`}
