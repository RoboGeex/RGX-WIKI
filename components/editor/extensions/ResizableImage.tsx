...
function normalizeCaptionHtml(value: string): string {
...
const ResizableImageComponent = (props: any) => {
  ...
  const [caption, setCaption] = useState(normalizeCaptionHtml(node.attrs.title || ''))
  ...
  useEffect(() => {
    ...
    if (!isCaptionFocused) {
      setCaption(normalizeCaptionHtml(node.attrs.title || ''))
    }
    setLayoutMode(node.attrs.layoutMode || 'fit')
  }, [node.attrs.width, node.attrs.textAlign, node.attrs.title, node.attrs.layoutMode, isCaptionFocused])
          ...
          <NodeViewErrorBoundary>
            <RichCaptionInput
              ...
              onChange={(html) => {
                ...
                updateAttributes({
                  title: html || '',
                  // Keep alt in sync for legacy data paths that still read from alt.
                  alt: plain || undefined,
                })
              }}
              onCommit={(html) => {
              ...
              onFocusChange={setIsCaptionFocused}
