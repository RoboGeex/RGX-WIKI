...
// The layout renders the wiki side panel; this index route just drops you
// into the first wiki you can manage.
export default async function EditorDashboardPage() {
  const sidebar = await getEditorSidebarData()

  if (sidebar.wikis.length > 0) {
    redirect(`/editor/${sidebar.wikis[0].slug}`)
  }

  if (sidebar.hasIdentity) {
    // Logged in but no manageable wikis: superadmins can create one from the
    // side panel.
    return (
